...
\leanok
任意の孤立超曲面根環は、原始重み、$|\det E|=h$、$h-\sum_iw_i=a$ を満たす可逆多項式による実際の次数付き表示を持つ。四変数以上では、他の正の根指数は全て $a$ を割り切り、したがって $a$ 以下である。
\end{theorem}
\begin{proof}
\leanok
\uses{thm:ternary-model,thm:higher-criterion,def:invertible}
三変数では実現済みの Cox 候補、四変数以上では Fermat 表示を用いる。重みの原始性と defect の等式は根環の Hilbert 級数から従う。高次元では signature index が $a$ に等しく、根の算術から整除性と最大性が従う。
\end{proof}
...
\leanok
原始重み、$|\det E|=h$、$h-\sum_iw_i=a>0$ を満たす全ての三変数可逆多項式は、その多項式そのもの、重み、関係式次数を持つ根環の表示として実現される。非零係数は任意である。
\end{theorem}
\begin{proof}
\leanok
...
\leanok
\texttt{theorem\_1\_4} の六つの連言は可逆性定理を形式化する。全次元での主可逆表示、任意の三変数可逆多項式に対する逆方向、置換を除く三変数 signature の一意性、根の一意性、高次元の算術的判定、および根環とその表示に関する結論を含む。Gorenstein parameter の記号は一貫して $a$ とする。
\end{theorem}
\begin{proof}
\leanok
\uses{thm:principal-presentation,thm:invertible-converse,thm:manuscript-rigidity,thm:root-arithmetic}
前述の表示・逆方向・剛性の結果と根の一意性を合わせる。高次元での存在条件は、指数の対ごとの互いに素性と $\prod_i p_i-\sum_i\prod_{j\ne i}p_j=a$ である。同じ環の任意の方程式に主行列式条件を課す定式化にはしていない。
\end{proof}
